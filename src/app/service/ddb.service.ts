...
export class DynamoDBService {
    ...
    writeInvite(data: string, date: string, type: string): void {
        console.log("DynamoDBService: writing " + type + " entry");
        var DDB = new DynamoDB({
            params: {TableName: "CUST_INVITE_CODES"}
        });

        // Write the item to the table
        var itemParams = {
        TableName: "CUST_INVITE_CODES",
        Item: {
            TO_MOBILE_NUMBER: { S: "9848055555" },
            EXPIRED: { BOOL: true },
            FROM_TCID: { S: "3f6835a6-a8d9-4afd-a856-ce306abcdbef" },
            INVITE_CODE: { S: "njhUksanlba" },
            NEW_CUST_TCID: { S: " " },
            INVITE_POINTS: { N: "0" },
            BOOKING_ID: { S: " " },
        ...
        ExpressionAttributeValues: {
            ":toNumber": {S: "9848055555"}
        },
        ConditionExpression: "NOT(TO_MOBILE_NUMBER = :toNumber)"
        };
        DDB.putItem(itemParams, function (result) {
            console.log("DynamoDBService: wrote entry: " + JSON.stringify(result));
        });
